...
export class DataProvider {
  ...
  loadCandidates() {
    ...
    return new Promise(resolve => {
      this.http.get("api/resources/candidates/all.json").subscribe (data => {
    ...
    if (boundary == "nation") return "Sierra Leone";
      ...
      switch (boundary) {
        case "district":
          ...
          break;
        case "constituency":
          boundary_field = "field_lt_constituency";
        case "ward":
  ...
  getDataByFields(election_results, parties, candidates, fields) {
    var result = {boundaries: [], total_votes: 0, invalied_votes: 0, parties: {}, candidates: {}};
    var boundary_results = this.getResultsByBoundary(election_results, fields.region);

    var parties_json = this.toPartiesJson(parties);
    var candidates_json = this.toCandidatesJson(candidates);
    
    var result_temp_parties = {}, result_temp_candidates = {}, result_temp_boundaries = {}, temp_candidates = {}, temp_parties = {};
    var missing_parties = [], missing_names = [];

    var party, votes, boundary, candidate_id, candidate_name, candidate_photo, candidate_percent, candidate_party_color, boundary_key, candidate_key;
    for (let election_result of boundary_results) {
      ...
      candidate_id = election_result['Candidate_SLEOP_ID'];

      if (candidate_name == " ") { missing_names.push(election_result); continue; }
      if (!result_temp_candidates[candidate_id])
        result_temp_candidates[candidate_id] = candidates_json[candidate_id];
      ...
      if (boundary != "") {
        if (!result_temp_boundaries[boundary_key]) {
          ...
          result_temp_boundaries[boundary_key]['candidates'][party] = { votes: votes, name: candidate_name, photo: candidate_photo, percent: candidate_percent, party: party, color: candidate_party_color, candidate: candidate_id };
    ...
    result['parties'] = result_temp_parties;
    result['candidates'] = result_temp_candidates;
  ...
  loadResultsByFields(fields) {
    ...
    return new Promise(resolve => {
      waterfall([
        ...
        function(parties, candidates, callback) {
          switch (fields.type) {
            case "president":
              vm.loadPresidentResultsByYear(fields.year).then(data => {
                callback(null, parties, candidates, data);
              })
              break;
            case "parliament":
            ...
            case "villageheadman":
              ...
              break;
            default:
              callback(parties, candidates, [], callback);
              break;
          }
        },
        function(parties, candidates, results, callback) {
          var result = {};
          ...
          var calculate = vm.getDataByFields(results, parties, candidates, fields);
          ...
          result['Candidates'] = calculate.candidates;
